 ...
 * @author Usuario
 */
public class TestBarberiaRunnable {
    ...
    Thread[] hiloCliente= new Thread[15];
        for (int i = 0; i < hiloCliente.length; i++) {
            hiloCliente[i]=new Thread(new Cliente(("Cliente-"+i),unaBarberia));
            hiloCliente[i].start();
        try {
            Thread.sleep((int)Math.random()*550);
        } catch (InterruptedException ex) {
            Logger.getLogger(TestBarberiaRunnable.class.getName()).log(Level.SEVERE, null, ex);
        }
        }
